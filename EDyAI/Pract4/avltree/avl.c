...
/**
 * @brief Funcion interna que balancea un arbol dada su raiz
 * 
 * @param raiz 
 * @return AVL_Nodo* 
 */
static AVL_Nodo* avl_balanceo_aux(AVL_Nodo* raiz) {
...
/**
 * @brief Balancea un arbol despues de producirse un desbalance de 1 nodo
 * 
 * @param arbol 
 * @return AVL 
 */
AVL avl_balanceo(AVL arbol) {
  arbol->raiz = avl_balanceo_aux(arbol->raiz);
...
/**
 * @brief Funcion interna para calcular el nodo mas a la izquierda de raiz-der
 * 
 * @param raiz 
 * @return AVL_Nodo* 
 */
static AVL_Nodo* minDerecha(AVL_Nodo* raiz) {
...
/**
 * @brief Funcion interna para eliminar un nodo de un arbol dada su raiz
 * 
 * @param raiz 
 * @param dato 
 * @param destruir 
 ...
 * @return AVL_Nodo* 
 */
static AVL_Nodo* avl_eliminar_aux(AVL_Nodo* raiz, void* dato, 
                            FuncionDestructora destruir, FuncionComparadora comparar, FuncionCopiadora copiar) {
  if (raiz == NULL)
    return NULL;

  if (comparar(dato, raiz->dato) < 0) {
    raiz->izq = avl_eliminar_aux(raiz->izq, dato, destruir, comparar, copiar);
  }
  else if (comparar(dato, raiz->dato) > 0) {
    raiz->der = avl_eliminar_aux(raiz->der, dato, destruir, comparar, copiar);
  }
  else {
    ...
    AVL_Nodo* temp = minDerecha(raiz->der);

    destruir(raiz->dato);
    raiz->dato = copiar(temp->dato);

    raiz->der = avl_eliminar_aux(raiz->der, raiz->dato, destruir, comparar, copiar);
  }
...
/**
 * @brief Elimina un dato de un AVL
 * 
 * @param arbol 
 * @param dato 
 * @return AVL 
...
AVL avl_eliminar(AVL arbol, void* dato) {
  ...
  arbol->raiz = avl_eliminar_aux(arbol->raiz, dato, arbol->destr, arbol->comp, arbol->copia);
